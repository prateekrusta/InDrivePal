...
const FindDriver = () => {
  const [fare, setFare] = useState(156); // Initial fare value is set to 156

  const handleIncrement = () => {
    setFare(prevFare => prevFare + 10); // Increment fare by 10
  };

  const handleDecrement = () => {
    setFare(prevFare => prevFare - 10); // Decrement fare by 10
  };
